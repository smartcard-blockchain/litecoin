...
LITECOIND=${LITECOIND:-$SRCDIR/smartcardcoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/smartcardcoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/smartcardcoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/smartcardcoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
SCCVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
