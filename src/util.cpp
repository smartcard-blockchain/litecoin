...
const char * const BITCOIN_CONF_FILENAME = "smartcardcoin.conf";
const char * const BITCOIN_PID_FILENAME = "smartcardcoind.pid";
...
#else
    const char* pszModule = "smartcardcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Smartcardcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Smartcardcoin";
#else
    // Unix
    return pathRet / ".smartcardcoin";
#endif
#endif
}
